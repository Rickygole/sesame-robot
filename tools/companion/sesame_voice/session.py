...
entrypoint (run_voice.py). It only knows about core/ (parse, resolve,
plan, expression) and a robot object with a mock_robot.MockRobot-shaped
interface (send/tick). It has no idea whether the text it's fed came from
a keyboard or a microphone, which is the whole point of the split: voice
is a front end that only produces text, not a second implementation of
what the text means.
"""
...
from .core import dialogue, expression, parser, plan
